...
def diagnostics_np(net_vel, true_vel, save_dir='./diagnosticsArvind/', iteration=2, pos=[0, 0, 2], dx=[0.1, 0.1, 0.1],
                   diagnostics=['spectrum', 'intermittency', 'structure_functions', 'QR', 'image']):
    ...
    if 'spectrum' in diagnostics:
        ...
        plt.loglog(x, net_e[1:], label='model flow')
    ...
    if 'intermittency' in diagnostics:
        ...
        np.savez(save_path + 'intermittency', net_ebins=net_ebins, net_zdns_plot=net_zdns_plot, true_ebins=true_ebins,
                 true_zdns_plot=true_zdns_plot)
        plt.semilogy(net_ebins, net_zdns_plot, label='model flow')
    ...
    if 'QR' in diagnostics:
        print('computing QR')
        net_q, net_r = q_r_np(net_vel, dx, coarse_grains=[1, 8, 32])
        true_q, true_r = q_r_np(true_vel, dx, coarse_grains=[1, 8, 32])
        np.savez(save_path + 'qr_data', net_q_0=net_q[1], net_q_8=net_q[8], net_q_32=net_q[32],
                 net_r_0=net_r[1], net_r_8=net_r[8], net_r_32=net_r[32],
                 true_q_0=true_q[1], true_q_8=true_q[8], true_q_32=true_q[32],
                 true_r_0=true_r[1], true_r_8=true_r[8], true_r_32=true_r[32])
